 ...
 if($(window).width() >=1024 ){	
  ...
  window.addEventListener('deviceorientation', function(event) {
    var alpha = event.alpha,
        beta = event.beta,
        gamma = event.gamma;
	$(".building").css("transform","translate3d("+gamma+"%,"+beta+"%,0)");	
	$(".green01").css("transform","translate3d("+gamma+"%,"+beta+"%,0)");	
	$(".green02").css("transform","translate3d("+gamma+"%,"+beta+"%,0)");	
	$(".rainbow").css("transform","translate3d("+gamma+"%,"+beta+"%,0)");	
	$(".mypic").css("transform","translate3d("+gamma+"%,"+beta+"%,0)");	
	$(".train").css("transform","translate3d("+gamma+"%,"+beta+"%,0)");	
	$(".yellowline").css("transform","translate3d("+gamma+"%,"+beta+"%,0)");	
